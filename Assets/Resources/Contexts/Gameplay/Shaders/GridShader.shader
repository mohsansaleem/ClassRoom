...
Shader "PDT Shaders/Map Grid" {
	Properties {
		...
		[IntRange] _SelectedCellXStart("Selected Cell X Start", Range(0,100)) = 0.0
		[IntRange] _SelectedCellXCount("Selected Cell X Count", Range(0,100)) = 0.0
		[IntRange] _SelectedCellYStart("Selected Cell Y Start", Range(0,100)) = 0.0
		[IntRange] _SelectedCellYCount("Selected Cell Y Count", Range(0,100)) = 0.0
	}
	SubShader {
		...
		float _SelectCell;
		float _SelectedCellXStart;
		float _SelectedCellXCount;
		float _SelectedCellYStart;
		float _SelectedCellYCount;
		...
		void surf (Input IN, inout SurfaceOutputStandard o) {
			...
			_SelectedCellXStart = floor(_SelectedCellXStart);
			_SelectedCellXCount = floor(_SelectedCellXCount);
			_SelectedCellYStart = floor(_SelectedCellYStart);
			_SelectedCellYCount = floor(_SelectedCellYCount);
			...
			if (round(_SelectCell) == 1.0 &&
				id.x >= _SelectedCellXStart && id.x < (_SelectedCellXStart + _SelectedCellXCount) &&
				id.y >= _SelectedCellYStart && id.y < (_SelectedCellYStart + _SelectedCellYCount))
			{
